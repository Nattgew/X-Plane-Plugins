...
char * str_append ( const char *target, const char *append ) {
	char * new = malloc(snprintf(NULL, 0, "%s~%s", target, append) + 1);
	sprintf(new, "%s~%s", target, append);
	return new;
}

char * flt_append ( const char *target, const float append ) {
	char * new = malloc(snprintf(NULL, 0, "%s~%.0f", target, append) + 1);
	sprintf(new, "%s~%.0f", target, append);
	return new;
}

char * lu_append ( const char *target, const long unsigned append ) {
	char * new = malloc(snprintf(NULL, 0, "%s~%lu", target, append) + 1);
	sprintf(new, "%s~%lu", target, append);
	return new;
}
