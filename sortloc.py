...
def getNeighbors(dataset, testInstance, k): #Get k nearest neighbors
	...
	if len(dataset)<k:
		...
		neighbors=[]
		for x in range(k):
			print("Adding neighbor "+str(distances[x][2])+": "+str(distances[x][0][0])+", "+str(distances[x][0][1]))
...
def getseeds(dataset, seeds, k):
	...
	for i in range(k):
		...
		for x in range(i,k):
			...
			if len(seeds)-1<x or len(seeds)==0:
